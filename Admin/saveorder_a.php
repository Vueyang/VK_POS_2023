...
<?php
	...
	FROM tbl_order 
	WHERE mem_id='$mem_id'";
	$query2	= mysqli_query($conn, $sql2) or die ("Error : ".mysqlierror($sql2));
	...
	foreach($_SESSION['cart'] as $p_id=>$qty)
	{
		...
		$query3	= mysqli_query($conn, $sql3) or die ("Error : ".mysqlierror($sql3));
		...
		$query4	= mysqli_query($conn, $sql4) or die ("Error : ".mysqlierror($sql4));
		//echo $sql4;
		//ตัดสต๊อก
		  for($i=0; $i<$count; $i++){
		  $have =  $row3['amount'];
		  ...
		  //echo "<hr>";
			$sql5 = "UPDATE product_new SET  
		     amount=$stc
		     WHERE  pro_id=$p_id ";
		  $query5 = mysqli_query($conn, $sql5) or die ("Error : ".mysqlierror($sql5));
		  }
  		/*   stock  */
	}
	//exit();
