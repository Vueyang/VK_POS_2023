	...
	<section class="content ">
		<div class="card card-gray l">
			<div class="card-header ">
				<h3 class="card-title" style="font-size: 2rem;">ຂໍ້ມູນລາຍຮັບ</h3>
			</div>
			<br>
					...
					<?php
						...
						$row = mysqli_fetch_row($nquery);
							...
							if (($dt1 != "") & ($dt2 != "")) {
								...
								$query = "
									SELECT order_id, pay_amount, pay_amount2, SUM(pay_amount2) AS pay_amount2, DATE_FORMAT(order_date, '%d-%M-%Y') AS order_date
									FROM tbl_order_receive WHERE order_date BETWEEN '$dt1' and '$date_to_date'
									GROUP BY DATE_FORMAT(order_date, '%d%')
									ORDER BY DATE_FORMAT(order_date, '%Y-%M-%d')  DESC $limit
           ...
           <div align="center" class="card-header">
				<div class="row">
					<div class="col-md-12" style="display:flex;" >
						...
						<h4 style=" margin: 10px;">ລາຍການຂໍ້ມູນລາຍຮັບ</h4>
						</div>
					</div>
				</div>
			</div>
                ...
                beginAtZero:true
						...
						$row = mysqli_fetch_row($nquery);
							...
							if (($dt1 != "") & ($dt2 != "")) {
								...
								$nquery_1 = mysqli_query($conn, "SELECT * from  tbl_order_receive WHERE order_date BETWEEN '$dt1' and '$date_to_date' ORDER BY order_id DESC $limit");
								
							}else{
								//$limit = 'LIMIT ' . ($pagenum - 1) * $page_rows . ',' . $page_rows;
								$nquery_1 = mysqli_query($conn, "SELECT * from  tbl_order_receive ORDER BY order_id DESC $limit");
								$paginationCtrls = '';
								if ($last != 1) {
									if ($pagenum > 1) {
										$previous = $pagenum - 1;
